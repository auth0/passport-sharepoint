...
function Strategy(options, verify) {
  ...
  this._spSiteUrl = options.spSiteUrl;
}
...
Strategy.prototype.authenticate = function(req, options) {
  ...
  if(spSiteUrl == undefined){
      spSiteUrl = options.spSiteUrl || this._spSiteUrl;
  }
  ...
  if (!spSiteUrl){ return self.error(new InternalOAuthError('SharePointStrategy requires a spSiteUrl.')); }
...
Strategy.prototype.userProfile = function(accessToken, spSiteUrl, done) {
  ...
  var options = {
    ...
    headers : headers,
    agent: false,
    // ciphers: 'RC4', RC4 cipher is not supported anymore from MS
